/** * Created by OXOYO on 2019/7/1. * * ContextMenu 右键菜单 */
      ...
      <template v-for="(item, index) in contextMenuList">
        ...
        >
          <template v-slot:label>
            ...
            <span class="item-shortcut" v-if="item.shortcuts">{{
              item.shortcuts.label
            }}</span>
          </template>
        </ToolItem>
        <ToolItem
            ...
            <ToolBox mode="vertical">
              <template v-for="(child, childIndex) in item.children">
                <ToolItem
                  ...
                  @click="handleChildClick(item, child, childIndex)"
                >
                  <template v-slot:label>
                    <template v-if="child.type === 'normal'">
                      ...
                      <span class="item-label">{{
                        child.lang ? $t(child.lang) : child.label
                      }}</span>
                    </template>
                ...
                <XDivider
                  ...
                  v-if="child.divider"
                  :key="
                    'contextmenu' + '_divider_' + index + '_child_' + childIndex
                  "
                  mode="horizontal"
                />
              </template>
  ...
  methods: {
    handleContextMenuList() {
      ...
      _t.toolList.forEach((item) => {
        if (
          ...
          item.enable &&
          item.contextmenu &&
          item.contextmenu.enable
        ) {
          if (
            !item.contextmenu.hasOwnProperty("target") ||
            (item.contextmenu.hasOwnProperty("target") &&
              item.contextmenu.target.length &&
              _t.options &&
              _t.options.type &&
              item.contextmenu.target.includes(_t.options.type))
          ) {
    ...
    handleContextMenuStyle() {
      ...
      _t.$nextTick(function () {
        const x =
          _t.options.x !== undefined
            ? parseInt(_t.options.x) > 0
              ? parseInt(_t.options.x)
              : 0
            : 0;
        const y =
          _t.options.y !== undefined
            ? parseInt(_t.options.y) > 0
              ? parseInt(_t.options.y)
              : 0
            : 0;
        // 判断是否超出边界
        if (
