...
def main(confFile):
    ...
    mode = -1
    modeLab = ["[clingo]", "[action theory]", "[context]", "[causal theory]", "[ethical spec]",
               "[ethical theories]", "[trace]", "[causal trace]", "[results]", "[output]"]
    action = []
    ctx = []
    causal = []
    ethSpec = []
    ...
    for l in lines:
        if l[0] == '[':
            ...
            if mode == modeLab.index("[clingo]"):
                ...
                output = l
    f.close()
    process(repclingo, action, ctx, causal, ethSpec,
            ethics, tr, ctr, resPred, output)
